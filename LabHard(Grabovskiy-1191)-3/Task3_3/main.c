...
static uint8_t k=0;																									//Переменная хранящая колличество символов, что нужно вывести в терминал PuTTY
...
// Выполнение арифметической прогрессии	
	...
	{														
		...
		i++;																														//Увеличить на один переменную отображающую количество итераций
	}
...
void debug(void)
{
	uint8_t mes_a[2]={0x61,0x3d};																				//Сообщения "a="
	uint8_t mes_n[2]={0x6e,0x3d};																				//Сообщения "n="
	uint8_t mes_q[2]={0x2d,0x31};																				//Сообщения "-1"
	uint8_t mes_s[2]={0x73,0x3d};																				//Сообщения "s="
	while(1)
	{
		if((flag&0xF8)>>3==31)																					 	//Если установлен флаг и нажата кнопка со сдвигом битов, то 
		{
			flag=0;																												 	//Флагу присваивается значение 0
			break;																												 	//Выход из уловия
		}
		
		if((flag&0x4)>>2==1)																						 	//Кнопка "i"
		{																			
			for(uint8_t v=0;v<2;v++)																				//Перебираем элементы с помощью цикла for
			{	
				while ((USART1->ISR & USART_ISR_TXE) == 0) {} 								//Дождаться готовности передатчика USART1 к приему битового пакета для отправки на ПК
				USART1->TDR = mes_n[v];																				//Вывод сообщения в окне терминала PuTTY значения переменной с номером итерации вычислений
			}
		...
		if((flag&0x2)>>1==1)																							//Кнопка "e"
		{																					
			for(uint8_t v=0;v<3;v++)																				//Перебираем элементы с помощью цикла for
			{	
				while ((USART1->ISR & USART_ISR_TXE) == 0) {}									//Дождаться готовности передатчика USART1 к приему битового пакета для отправки на ПК 						
				USART1->TDR = mes_a[v];																				//Вывод сообщения в окне терминала PuTTY значения переменной, соответствующим текущей итерации
			}
			while ((USART1->ISR & USART_ISR_TXE) == 0) {}										//Дождаться готовности передатчика USART1 к приему битового пакета для отправки на ПК 						
			...
			{	
				while ((USART1->ISR & USART_ISR_TXE) == 0) {}									//Дождаться готовности передатчика USART1 к приему битового пакета для отправки на ПК 						
				USART1->TDR = mes_q[v];																				//Вывод сообщения в окне терминала PuTTY значения элемента вычислений, соответствующим текущей итерации
			}
			while ((USART1->ISR & USART_ISR_TXE) == 0) {}										//Дождаться готовности передатчика USART1 к приему битового пакета для отправки на ПК 						
			USART1->TDR = 0x0D;																							//Переход на новую строку
			while ((USART1->ISR & USART_ISR_TXE) == 0) {}										//Дождаться готовности передатчика USART1 к приему битового пакета для отправки на ПК 						
		...
		if((flag&0x1)==1)																									//Кнопка "s"
		{
			for(uint8_t v=0;v<2;v++)																				//Перебираем элементы с помощью цикла for
			{	
				while ((USART1->ISR & USART_ISR_TXE) == 0) {}									//Дождаться готовности передатчика USART1 к приему битового пакета для отправки на ПК 						
				USART1->TDR = mes_s[v];																				//Вывод сообщения в окне терминала PuTTY текущего значения переменной с накапливаемым результатом вычислений
			}
...
{
	uint8_t j=k;																												//Записываем в переменную j значение хранящая колличество символов, что нужно вывести в терминал PuTTY
	uint8_t flag_0=0;																										//Переменная флага ноль в состоянии 0
	//Перебираем значения с помощью цикла for
	for(;num!=0;k++)
	{
		mes[k]=(uint32_t)num%10+0x30;																			//Записываем в буфер данных изменённое значение num , что передаёться на ПК посредством USART
		num=num/10;																												//Изменяем значение num, чтобы пройтись и записать всё число 
	}
	//Цикл для вывода сообщения в терминала PuTTY
	for(uint8_t d=0;k-j>=d;d++)
	{
			flag_0=1;																												//Переводим флаг ноль в состояние 1
		
			//Дождаться готовности передатчика USART1 к приему битового пакета для отправки на ПК
			while ((USART1->ISR & USART_ISR_TXE) == 0) {} 						 						
			USART1->TDR = (uint8_t)mes[k-d];																//Вывод сообщения в окне терминала PuTTY
	}
